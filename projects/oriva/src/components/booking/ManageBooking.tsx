...
import { CURRENCY, clinic, services, staff } from '../../data/clinic';
import {
  formatDateLong,
  formatDuration,
  formatPrice,
    ...
    <AnimatePresence>
      {open ? (
        ...
        >
          ...
          >
            <Panel initial={reference} onClose={onClose} />
          </motion.div>
        </motion.div>
      ) : null}
    </AnimatePresence>
...
function Panel({ initial, onClose }: { initial: string; onClose: () => void }) {
  const { bookings, setStatus, reschedule } = useBookings();
  const reduce = useReducedMotion();

  const [input, setInput] = useState(initial);
  const [lookup, setLookup] = useState<string | null>(initial ? initial.toUpperCase() : null);
  const [error, setError] = useState<string | null>(null);
  const [confirmingCancel, setConfirmingCancel] = useState(false);
  const [rescheduling, setRescheduling] = useState<Booking | null>(null);
  // Captured once, when the panel opens — the cutoff must not shift mid-render.
  const [openedAt] = useState(() => Date.now());

  const booking = lookup ? (bookings.find((b) => b.id === lookup) ?? null) : null;
  ...
  function isInsideCutoff(b: Booking) {
    const start = fromISODate(b.date);
    start.setMinutes(timeToMinutes(b.time));
    return start.getTime() - openedAt < 24 * 60 * 60 * 1000;
  }
  ...
  const locked =
    booking !== null &&
    (booking.status === 'cancelled' || booking.status === 'completed' || booking.date < todayISO());
          ...
          {booking && service && member ? (
            ...
            >
              <div className="flex items-start justify-between gap-4">
                <div>
                  <p className="font-display text-[22px] leading-tight text-ink-900">{service.name}</p>
              ...
              <dl className="mt-5 divide-y divide-line overflow-hidden rounded-2xl border border-line">
                {[
                  {
                    k: 'When',
                    v: `${formatDateLong(booking.date)}, ${minutesToLabel(timeToMinutes(booking.time))}`,
                  },
                ...
                <>
                  {isInsideCutoff(booking) ? (
                    <p className="mt-5 flex gap-2 rounded-2xl bg-copper-200/40 px-4 py-3.5 text-[12.5px] leading-relaxed text-copper-700">
                      ...
                      You are inside the 24-hour window. Changes here still work, but the clinic retains
                      50% of the fee — call us on {clinic.phoneDisplay} and we will do our best.
                    </p>
                  ) : null}
