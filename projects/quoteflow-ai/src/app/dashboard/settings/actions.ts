...
export async function updateBusinessAction(_prev: FormState, formData: FormData): Promise<FormState> {
  ...
  const parsed = businessSettingsSchema(d).safeParse(Object.fromEntries(formData));
...
export async function updatePublicPageAction(_prev: FormState, formData: FormData): Promise<FormState> {
  const { dict: d } = await getI18n();
  const parsed = publicPageSettingsSchema(d).safeParse({
  ...
  const result = await runAsMember(async (tx, ctx) => {
    const v = parsed.data;
    if (v.slug !== ctx.workspace.slug && !(await slugAvailable(tx, v.slug, ctx.workspace.id))) return { error: d.settings.slugTaken };
    await updateWorkspace(tx, ctx.workspace.id, { slug: v.slug });
    await updateSettings(tx, ctx.workspace.id, {
      public_page_enabled: v.public_page_enabled,
      public_page_headline: v.public_page_headline,
      public_page_intro: v.public_page_intro,
      public_page_thank_you: v.public_page_thank_you,
    });
