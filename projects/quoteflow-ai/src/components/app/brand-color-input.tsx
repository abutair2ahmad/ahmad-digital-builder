    ...
    <div className="space-y-2">
      <div className="flex items-center gap-2">
        <label className="relative size-9 shrink-0 overflow-hidden rounded-md border" style={{ backgroundColor: safeHex(value) }}>
          <input type="color" aria-label="Pick a colour" value={safeHex(value)} onChange={(e) => setValue(e.target.value)} className="absolute inset-0 size-full cursor-pointer opacity-0" />
        </label>
        <Input name={name} value={value} onChange={(e) => setValue(e.target.value)} pattern="^#[0-9a-fA-F]{6}$" className="font-mono" />
      </div>
      <div className="flex flex-wrap gap-1.5">
        {PRESETS.map((c) => (
          <button key={c} type="button" aria-label={`Use ${c}`} onClick={() => setValue(c)} className="size-6 rounded-full border ring-offset-2 focus-visible:ring-2" style={{ backgroundColor: c, outline: value === c ? `2px solid ${c}` : undefined, outlineOffset: 2 }} />
        ))}
      </div>
    </div>
  );
