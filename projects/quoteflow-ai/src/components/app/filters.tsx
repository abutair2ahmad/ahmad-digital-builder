...
export function SearchBox({ placeholder }: { placeholder: string }) {
  ...
  return (
    <div className="relative w-full sm:w-72">
      ...
      <Input value={value} onChange={(e) => setValue(e.target.value)} placeholder={placeholder} className="pl-8" aria-label="Search" />
    </div>
  );
}
    ...
    <div className="flex flex-wrap gap-1.5">
      {options.map((o) => {
        ...
        return (
          ...
          >
            {o.label}
            {o.count !== undefined ? <span className={cn('ml-1.5 tabular', active ? 'text-background/70' : 'text-muted-foreground/70')}>{o.count}</span> : null}
          </button>
        );
      })}
    </div>
