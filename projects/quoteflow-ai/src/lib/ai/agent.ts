 ...
 */
export async function nextAgentTurn(ctx: AgentContext, history: ConversationTurn[], collected: Collected): Promise<AgentTurn> {
  if (hasAnthropic) {
    try {
      const { claudeTurn } = await import('./claude');
      return await claudeTurn(ctx, history, collected);
    } catch (err) {
      console.error('[ai] Claude turn failed, falling back to guided flow:', err);
    }
  }
  return scriptedTurn(ctx, history, collected);
}
